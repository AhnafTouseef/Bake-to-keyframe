...
class REAL_PT_testpanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        try:
            ...
            row = layout.row(align=True)
            row.scale_y=2
            row.operator('poke.mon' , text ='Bake to Keyframe', icon = 'EVENT_B')
            row.scale_x=2
            row.operator('object.shape_key_remove' , icon = 'EVENT_D', text='').all= True

            
            col = layout.column(align=True)
            col.scale_y=2
            col.operator('place.keyframe' , text ='Place Keyframe', icon = 'EVENT_A')
            col.operator('recalculate.keyframe' , text ='Recalculate Keyframe', icon = 'EVENT_R')
